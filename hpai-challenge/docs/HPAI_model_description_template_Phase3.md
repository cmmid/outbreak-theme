...
You may indicate only the changes since the previous phase, describe the modifications made, or, if nothing has changed, state this here.
...
| :---- | :---- | :---- |
| *Example: Susceptible* | *S* | *Individuals that are healthy and can become infected* |
...
* ***A general description of the apparently-resolved epidemic, including a table describing the distribution of the outbreaks per species and production types, a timeline of the incidence and a visual representation of the spatial distribution of the outbreaks. (please also provide the visual representation as a separate file in pdf format).***

	*\[to fill\]*

* ***An answer with a justification to the following question: when could we have started restocking the infected farms that have been depopulated, without causing a rebound of the epidemic?*** 

	*\[to fill\]*

* ***An answer with a justification to the following question: how many outbreaks were averted (in total) thanks to the implementation of the preventive culling strategy?*** 

	*\[to fill\]*

* ***An answer with a justification to the following question: what should have been the daily culling capacity at the end of phase 1 to ensure (i.e. be 95% sure) virus eradication by the end of phase 2?*** 
